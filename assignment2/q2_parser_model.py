...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder = tf.placeholder(dtype=tf.int32, shape=(None, self.config.n_features), name='input_placeholder')
        self.labels_placeholder = tf.placeholder(dtype=tf.float32, shape=(None, self.config.n_classes), name='labels_placeholder')
        self.dropout_placeholder = tf.placeholder(dtype=tf.float32, shape=(), name='dropout_placeholder')
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeddings_ = tf.nn.embedding_lookup(params=self.pretrained_embeddings, ids=self.input_placeholder)
        embeddings = tf.reshape(embeddings_, shape=(-1, self.config.n_features * self.config.embed_size), name='embeddings')
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        W = xavier_weight_init()((self.config.n_features * self.config.embed_size, self.config.hidden_size), name='W')
        b1 = tf.Variable(tf.zeros(shape=(self.config.hidden_size,)), name='b1')

        z = tf.add(tf.matmul(x, W), b1)
        h = tf.maximum(tf.constant(0., dtype=tf.float32), z)
        h_drop = tf.nn.dropout(h, keep_prob=self.dropout_placeholder)


        U = xavier_weight_init()((self.config.hidden_size, self.config.n_classes), name='U')
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        xentropy = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred, name='xentropy')
